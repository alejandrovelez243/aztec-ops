...
Two families, and the boundary between them is the point of the whole context. **Obedience** is
``transition``: whether the record in front of you may make the move it is asking for, decided
against the rows every time it is asked. **Authoring** is the other eight: what shape the graph has
at all. Authoring writes what obedience reads, and never the other way round — no authoring module
imports ``transition``, and ``transition`` imports none of them.
