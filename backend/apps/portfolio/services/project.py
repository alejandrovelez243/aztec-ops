...
The three project use cases live one per module — ``create_project.py``, ``update_project.py``,
``transition_project.py`` — because each is a transaction boundary and BACKEND §6 keeps one public
function per module. This module is the single import point other contexts and the API router use,
so a use case can be split or renamed without every caller moving.
