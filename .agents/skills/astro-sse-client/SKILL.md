...
Normative for every file in `frontend/`: `docs/standards/FRONTEND.md` and
`docs/standards/PATTERNS_FRONTEND.md`. The rules that bite hardest in this skill's area:

- **One `EventSource`, in the store** (FRONTEND §5 SRP, §6; PATTERNS §2). `new EventSource(` and
  bare `fetch(` exist only inside `src/lib/stream/` and `src/lib/api/`; a lint rule bans them
  elsewhere. `connect()` appears exactly once in the codebase, in `Shell.astro`. Every
  `subscribe()` return value is stored and called on teardown (`astro:before-swap`).
- **View state is a discriminated union, switched exhaustively** (FRONTEND §1; PATTERNS §4). No
  `isLoading` next to `hasError`. The union lives in `src/lib/view-state.ts`, `disconnected`
  carries the last data plus `lastEventAt`, and every `switch` ends in `assertNever` so a new arm
  fails the build.
- **Types are generated, taxonomies are data** (FRONTEND §1, §5 OCP; PATTERNS §7). `src/lib/api/types.ts`
  is regenerated by `npm run gen:api` and committed unedited; domain aliases derive from
  `components["schemas"][...]`. No `any`, no `as`, no `!`. No object literal in `frontend/src` keyed
  by a state, risk, signal or priority code, and no `switch` on `state.code` — only on
  `state.category` or `flag.severity`.

Before a PR: `npx astro check`, `npx eslint .`, `npx prettier --check .`, and
`npm run gen:api && git diff --exit-code src/lib/api/types.ts`.
...
- Discarding the `subscribe()` return value, so an island that remounts across an Astro view
  transition accumulates handlers and patches detached DOM nodes.
- Tracking view state with `isLoading` / `hasError` / `items.length === 0` instead of the union in
  `lib/view-state.ts`, or writing a `switch (view.kind)` with a `default:` that swallows a new arm
  instead of an `assertNever` that fails the build.
- Hand-writing an `interface` for a response shape that already exists in the generated
  `api/types.ts`, or editing that file directly. The fix is a backend schema change plus
  `npm run gen:api`.
- Reaching for `as`, `any` or `!` to get past `noUncheckedIndexedAccess` — `queue.items[0]!`
  asserts away the empty-queue view state the flag exists to force you to render.
- Applying a patch without comparing the envelope's `occurred_at` against the row's rendered
  `updated_at`, or rebuilding the whole matrix from a payload that only carries the change.
- Painting the new state before the transition POST returns. A `revert()` or `previousState` in an
  island means something was painted too early; the button shows pending until
  `project.state_changed` arrives.
- A hex, an `rgb(`, an off-scale `px` or a non-zero `border-radius` in a component. Colour and
  spacing come from `styles/tokens.css`; status colour comes from `category` or `severity`.
- JSDoc that restates the signature ("Connects to the stream") instead of stating the invariant and
  the failure mode — the refcount, the dedupe on `event.id`, what leaks if cleanup is skipped.
